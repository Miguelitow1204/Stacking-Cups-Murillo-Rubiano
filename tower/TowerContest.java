...
public class TowerContest {
    ...
    private List<Integer> findSubsetSum(List<Integer> contribs, long target) {
        ...
        if (findSubsetHelper(contribs, 0, target, result))
            return result;
        return null;
    }
    ...
    private boolean findSubsetHelper(List<Integer> contribs, int index,
        ...
        if (remaining == 0)
            return true;
        if (remaining < 0 || index >= contribs.size())
            return false;
 
        // Usar valor directamente como contribucion
        int val = contribs.get(index);
        current.add(val);
        if (findSubsetHelper(contribs, index + 1, remaining - val, current))
            return true;
    ...
    private String buildOrdering(int n, List<Integer> contributors) {
        ...
        // Copa n siempre primero(Es el contenedor principal)
        order.add(n);
        // contributord ascendente -> se anida dentro de copa n
    ...
    public void simulate(int n, int h) {
        ...
        // Calcular ancho adecuado basado en la copa más grande
        int maxId = cupIds.stream().mapToInt(Integer::intValue).max().orElse(n);
        int towerWidth = 15 + (maxId * 3) + 20; // margen extra
 
        Tower tower = new Tower(towerWidth, h + 1);
        for (int cupId : cupIds) {
            tower.pushCup(cupId);
            Canvas.getCanvas().wait(500);// entre cada cup 500ms
        }
